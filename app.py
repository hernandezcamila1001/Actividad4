...
data_real_fit = data_real # Se crea una variable "data_real_fit" que contiene la información de "data_real".
...
x_lim = data_real['Esfuerzo (kN)'].iloc[-1] # Se define la variable "x_lim" donde se selecciona el último valor de la columna 'Esfuerzo (kN)', y se guarda.
y_lim = data_real['Deformación (mm)'].iloc[-1] # Se define la variable "y_lim" donde se selecciona el último valor de la columna 'Deformación (mm)', y se guarda.
...
data_list2 = lecturaDatos2() # Se define la variable "data_list2" que comprende la función "lecturaDatos2()", llamada desde "BD.baseDatos".
data_real2 = pd.DataFrame(data_list2) # Se define la variable "data_real2" que crea una base de datos de la información suministrada en la variable "data_list2".
data_real_fit = data_real2 # Se crea una variable "data_real_fit" que contiene la información de "data_real2".
...
dataRealEsfuerzo = data_real2['Esfuerzo (kN)'] # Se define la variable "dataRealEsfuerzo" donde se disponen los datos de la columna 'Esfuerzo (kN)' de "data_real2".
dataRealDeformacion = data_real2['Deformación (mm)']  # Se define la variable "dataRealDeformacion" donde se disponen los datos de la columna 'Deformación (mm)' de "data_real2".

""" Lectura de los gráficos. """

# Se llama a la función “gr_sin_prediccion2” que visualiza el gráfico del comportamiento de los datos reales y teóricos. 
gr_sin_prediccion2(dataTeoricoEsfuerzo,dataTeoricoDeformacion,dataRealEsfuerzo,dataRealDeformacion)
# Se llama a la función “gr_con_prediccion2” que visualiza el gráfico del comportamiento de los datos reales y teóricos, permitiendo ver un zoom de estos. 
gr_con_prediccion2(x_lim,y_lim,dataTeoricoEsfuerzo,dataTeoricoDeformacion,dataRealEsfuerzo,dataRealDeformacion)
# Se llama a la función “gr_con_prediccion2_3000” que visualiza el gráfico del comportamiento de los datos reales y teóricos, y la predicción del modelo para el dato de 3000 kN.
